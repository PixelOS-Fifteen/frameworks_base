...
/**
 * A wrapper interface to monitor the given set of {@link Settings.System}.
 */
@FunctionalInterface
interface SystemSettingsChangeCallback {
    /**
     * Called back when the value associated with {@code key} is updated.
     *
     * @param key a key defined in {@link Settings.System}
    ...
    /**
     * Registers {@link SystemSettingsChangeCallback} to the given set of {@link Settings.System}.
     *
     * @param handler  {@link Handler} to be used to call back {@link #onChange(String, int, int)}
     * @param resolver {@link ContentResolver} with which {@link Settings.System} will be retrieved
     * @param keys     A set of {@link Settings.System} to be monitored
     * @param callback {@link SystemSettingsChangeCallback} to be called back
     */
    @NonNull
    static void register(@NonNull Handler handler, @NonNull ContentResolver resolver,
            @NonNull String[] keys, @NonNull SystemSettingsChangeCallback callback) {
        final ArrayMap<Uri, String> uriMapper = new ArrayMap<>();
        for (String key : keys) {
            uriMapper.put(Settings.System.getUriFor(key), key);
        }
